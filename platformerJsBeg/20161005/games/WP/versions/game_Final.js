...
var _tileMap = [
  {
    ...
    "map": [
      ...
      [[0,8],[0,12],[1,4],[1,3],[1,4],[1,6],[1, 9],[0,3]],
      [[0,12],[4,4],[0,4],[0,13],[4,14],[4,5]],
      [[0,8],[4,2],[0,13],[0, 8],[0,11],[0,6]],
      [[4,1],[0,9],[0,37]],
      [[0,5],[4,5],[0,38]],
      [[1,48]]
    ]
  ...
  },
  {
    ...
    "map": [
      [[4,1],[0,1],[0,12],[0,1],[0,8],[0,1],[0,5],[0,1],[0,5],[0,1]],
      [[0,8],[0,1],[0,3],[0,1],[0,6],[0,1],[0,23]],
      [[0,8],[0,19],[0,1],[0,3],[0,1],[0,6],[0,1],[3,1],[0,8]],
      [[0,11],[0,1],[0,12],[0,1],[0,14],[3,2],[0,9]],
      [[0,14],[1,1],[2,3],[1,1],[0,15],[3,3],[0,11]],
      [[0,14],[1,1],[2,3],[1,1],[0,11],[3,4],[0,14]],
      [[0,14],[1,5],[0,6],[3,5],[0,18]],
      [[1,5],[2,15],[3,5],[2,23]]
    ]
  },
  {
  "level": 4,
  "map": [
    [[1,4],[0,2],[1,12],[1,10],[1,8],[1,8],[1,5],[1,4],[1,5],[1, 8]],
    [[0,4],[0,2],[0,7],[0,6],[0,6],[0, 8],[0,23]],
    [[0,4],[0,2],[0,4],[0,3],[0,4],[0,6],[0, 9],[0,16]],
    [[0,12],[0,4],[0,2],[8,13],[0,16],[0,5]],
    [[0,8],[0,2],[0,4],[8,4],[2,11],[0,19]],
    [[0,10],[4,3],[0,37]],
    [[4,5],[4,5],[0,38]],
    [[1,48]]
  ]
},
{
  ...
  "map": [
    [[0,1],[0, 8],[0,13],[0,4],[0,8],[0, 8],[0,5],[0,4],[0,5],[0, 8]],
    [[0,19],[0,1],[0,6],[0,4],[0,6],[0, 8],[0,12]],
    [[0,19],[1,1],[0,4],[0,3],[0,4],[0,6],[0, 9],[0,3]],
    [[0,19],[1,1],[0,7],[0,13],[4,21],[4,5]],
    [[0,19],[1,1],[0,13],[0, 8],[0,1],[0,6]],
    [[0,19],[1,1],[0,28]],
    [[0,19],[1,1],[0,28]],
    [[1,20],[2,28]]
  ]
...
var assets = {'tiles': ['img/tile-1.png', 'img/platform.png', 'img/platformx2.png', 'img/sun.png', 'img/tile-2.png','img/tile-3.png']};
var playerSprite = { 'sprites': { 'img/playerSprite.png': { tile: 67, tileh: 80, map: { man_left: [10, 0], man_right: [0, 0], jump_right: [7, 0] } } } };
...
function initialiseGame () {
  Crafty.load(assets, function(){
    ...
    Crafty.audio.add("marioTheme", "sounds/marioTheme.mp3");
...
function placeFinish(){
  ...
  Crafty.e('Flagpost, 2D, DOM, flag_post, Image, Collision')
    .attr({
      x: 47*TILE_WIDTH ,
      y: SCREENHEIGHT - TILE_HEIGHT - 268 ,
      w: 77 ,
      h: 268
    })
    .image('img/flag.png')
    .onHit('Player', function(hitData) {
      if( !_flagFinish ){
        Crafty.audio.stop('marioTheme');
        Crafty.audio.play('marioTheme');
...
function spawnPlayer (){
  _player = Crafty.e('Player, 2D, DOM, man_right, SpriteAnimation, Twoway, Collision, Gravity, Tween, Keyboard')
    ...
    .reel('moveRight', 200, [[0, 0],[1,0],[2,0],[3,0]])
    .reel('moveLeft', 200, [[10,0],[11,0],[12,0],[13,0]])
    .reel('moveJumpLeft',500, [[14,0],[15,0],[16,0],[17,0],[18,0],[19,0]])
    .twoway(200, 510)
    .gravity('FloorTile')
    ...
    .bind('CheckJumping', function (ground) {
      ...
      this.pauseAnimation();
      this.resetAnimation();
      this.animate('moveJumpLeft', 2)
      //this.sprite(this.currentDirection === RIGHT ? 6 : 0,0);
    })
    ...
    tileRow.map(function (tile, tileIdx) {
      ...
      if (tileType === 1) {
        ...
        if (tileType === 4) {
          ...
          for(var i=0; i < tileNum; i++){
            Crafty.e('FloorTile2, 2D, DOM, Image')
              .attr({ x: xPos, y: yPos, w: TILE_WIDTH, h: TILE_HEIGHT })
              .image(Crafty.assets['img/tile-' + tileType + '.png'].src);
            xPos += 80;
          }
          ...
          for(var i=0; i < tileNum; i++){
            Crafty.e('FloorTile, 2D, DOM, Image')
              .attr({ x: xPos, y: yPos, w: TILE_WIDTH, h: TILE_HEIGHT })
              .image(Crafty.assets['img/tile-' + tileType + '.png'].src);
            xPos += 80;
          }
...
Crafty.bind('EnterFrame', function(){
  ...
  if( __STATE.level == 5 && !__STATE.bossSpawned && _player.x>(14 * TILE_WIDTH) ){
    Crafty.e('Boss');
    __STATE.bossSpawned = true;
  }
