...
var playerSprite = { 'sprites': { 'img/player.png': { tile: 68, tileh: 80, map: { man_still: [0,0], man_left: [0, 1], man_right: [0, 4], jump_right: [6, 4] } } } };
initialiseGame();
...
function initialiseGame () {
  Crafty.load(assets, function(){
    ...
    Crafty.audio.add("fire", "sounds/shoot.wav");
...
function spawnPlayer (){
  _player = Crafty.e('Player, 2D, DOM, man_still, SpriteAnimation, Twoway, Collision, Gravity, Tween, Keyboard')
    ...
    .reel('moveLeft', 500, [[0, 2], [1, 2], [2, 2], [3, 2], [0, 3]])
    .reel('still', 500, [[1,1]])
    .reel('jumpRight', 500, [[2,1], [3,1]])
    .reel('still', 500, [[1,1]])
    .reel('jumpLeft', 500, [[2,3], [3,3]])
    .twoway(200, 510)
    .gravity('FloorTile')
    ...
    .bind('KeyDown',
      function (e){
        if(e.key === Crafty.keys.SPACE){
          Crafty.audio.play('fire',1,1);
          if(this.currentDirection === RIGHT){
            Crafty.e('Bullet, 2D, DOM, Color, Tween')
              .attr({x: this.x, y: this.y + 50, w: 10, h: 10})
              .color('#FFFFFF')
              .tween({x:this.x + 5000}, 1000)
            }
      else{
        Crafty.e('Bullet, 2D, DOM, Color, Tween')
          .attr({x: this.x, y: this.y + 50, w: 10, h: 10})
...
function spawnThing(){
  ...
  .attr({x: 50,Y: 300, h: 100, w:100})
  .color('#000000')
  .gravity('FloorTile','playerSprite')
}
