...
var _tileMap = [
{
  ...
  'map': [
    ...
    [[0,15],[8,1],[0,1],[8,1],[0,19],[8,1],[0,11]],
    [[0,9],[4,1],[0,38]],
    [[0,48]],
    [[1,48]]
  ]
...
var playerSprite = { 'sprites': { 'img/Runner.png': { tile: 68, tileh: 80, map: { man_still: [0,0], man_left: [0, 1], man_right: [0, 2], jump_right: [6, 4] } } } };
...
function initialiseGame () {
  Crafty.load(assets, function(){
    ...
    displayText();
    /* Load sounds */
    Crafty.audio.add("coin", "sounds/Pickup_Coin.wav");
...
function spawnPlayer (){
  _player = Crafty.e('Player, 2D, DOM, man_still, SpriteAnimation, Twoway, Collision, Gravity, Tween, Keyboard')
    ...
    .reel('moveLeft', 500, [[0, 1], [1, 1], [2, 1], [3, 1], [4, 1], [5, 1]])
    .reel('still', 500, [[5, 2]])
    .reel('jump', 500, [[5, 2],[2, 2],[3,2]])
    .twoway(200, 510)
    .gravity('FloorTile')
    ...
    .bind('KeyDown',
      function (e) {
        ...
        if (e.key === Crafty.keys.RIGHT_ARROW && !this.isJumping) {
          ...
          var bullet = Crafty.e('Bullet, 2D, DOM, Tween, Color, Collision')
            .attr({x: this.x, y: this.y + 15, h: 10, w: 20})
            .color('#FF0000')
    ...
    /*function spawnThing (){
      thing = Crafty.e('2D, DOM, Color, Gravity, Collision')
...
function spawnWalkers () {
  Crafty.e('Delay').delay(spawnWalker, Crafty.math.randomInt(2000, 5000), -1);
}
...
function displayTimeLeft () {
  ...
  if(_livesLeftDisplay){
    _livesLeftDisplay.destroy();
  }
  ...
  _livesLeftDisplay = Crafty.e("2D, DOM, Text")
    .attr({ x: 10 - Crafty.viewport._x, y: 20 })
    .text(__STATE.livesLeft)
    .textColor('#FF0000')
    .textFont({ size: '14px', weight: 'bold', family: 'Courier New' });
}
...
function reset () {
  ...
  _timeLeftDisplay = "";
  _livesLeftDisplay = _livesLeftDisplay
  _secondsTotal = 120;
