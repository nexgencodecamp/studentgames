...
setupGlobalBindings();
var assets = {'tiles': ['img/tile-1.png', 'img/tronPlatform.png', 'img/tronPlatformx2.png']};
var playerSprite = { 'sprites': { 'img/goodGuy.png': { tile: 68, tileh: 80, map: { man_still: [0,0], man_left: [0, 1], man_right: [0, 2], jump_right: [0, 3] } } } };
var ninjastarSprite = { 'sprites': { 'img/trondiscsmall.png': { tile: 32, tileh: 32, map: { ninja_star: [0,0] } } } };
...
function loadBackground () {
  ...
  Crafty.background('#FFFFFF url(img/tronbackground.png) repeat-x center center');
}
...
function spawnPlayer (){
  _player = Crafty.e('Player, 2D, DOM, man_still, SpriteAnimation, Twoway, Collision, Gravity, Tween, Keyboard')
    ...
    .reel('moveLeft', 500, [[3, 1], [2, 1], [1, 1]])
    .reel('still', 500, [[1,2]])
    .reel('jump', 500, [[1, 2], [2, 2], [3, 2]])
    .twoway(200, 510)
    .gravity('FloorTile')
  ...
  tileMap.map(function (tileRow, rowIdx) {
    ...
    tileRow.map(function (tile, tileIdx) {
      ...
      if (tileType === 1) {
        ...
        if (tileType === 4) {
          Crafty.e('Platform')
            .setImage('img/tronPlatform.png')
            .setPlatform(xPos, yPos, 1)
            .addCoins(Crafty.math.randomInt(1,2));
        }
        else if (tileType === 8) {
          Crafty.e('Platform')
            .setImage('img/tronPlatformx2.png')
