...
var _secondsTotal = 150;
var _secondsTimeLimit = 150;
...
function loadBackground () {
  ...
  //Crafty.background('#FFFFFF url(img/scaryBackground.jpg) repeat-x center center');
  Crafty.e('BG, 2D, DOM, Image')
    .attr({ x: 0, y: 0, z: 0, w:3840, h:600 })
    .image('img/scaryBackground.jpg', 'repeat-x');
}
...
function spawnPlayer (){
  _player = Crafty.e('Player, 2D, DOM, man_right, SpriteAnimation, Twoway, Collision, Gravity, Tween, Keyboard')
    ...
    .reel('moveRight', 500, [[0, 0], [1, 0], [2, 0], [3, 0], [4, 0], [0, 1], [1, 1], [0, 1], [4, 0], [3, 0], [2, 0], [1, 0]])
    .reel('moveLeft', 500, [[1, 2], [2, 2], [3, 2], [4, 2], [0, 3], [1, 3], [2, 3]])
    .reel('jumpRight', 500, [[3, 1], [4, 1], [0, 2]])
    .reel('jumpLeft', 500, [[4, 3], [0, 4], [1, 4]])
    .reel('idleRight', 0, [[2, 1]])
    .reel('idleLeft', 0, [[3, 4]])
    .twoway(329, 510)
    .gravity('FloorTile')
...
function displayGameOver(){
  ...
  Crafty.e("GameOver, 2D, DOM, Text")
    ...
    .text("YOU GOT UTTERLY WRECKED").textColor('#FF0000').textFont({ size: '36px', weight: 'bold', family: 'Courier New' });
}
...
function displayGameComplete(){
  ...
  Crafty.e("GameComplete, 2D, DOM, Text")
    .attr({ x: 270 - Crafty.viewport._x, y: 280, z:100, w: 350 })
    .text("Game by Joshua").textColor('#33FF33').textFont({ size: '36px', weight: 'bold', family: 'Courier New' });
  Crafty.e("GameComplete, 2D, DOM, Text")
  .attr({ x: 270 - Crafty.viewport._x, y: 280, z:100, w: 350 })
  .text("Game by Joshua").textColor('#3FFFFF').textFont({ size: '30px', weight: 'bold', family: 'Courier New' });
