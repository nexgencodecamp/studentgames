...
function initialiseGame () {
  ...
  Crafty.audio.add("bgm", "sounds/ennio_morricone-the_good_the_bad_the_ugly.mp3");
    __STATE.gameStarted = true;

    if( __STATE.bgmPlaying == false ){
      Crafty.audio.play("bgm", -1, 1);
      __STATE.bgmPlaying = true;
    }
  });
}
...
function loadBackground () {
  ...
  Crafty.e('BG, 2D, DOM, Image')
    ...
    .image('img/bg_desert.jpg', 'repeat-x');
}
...
function spawnPlayer (){
  _player = Crafty.e('Player, 2D, DOM, man_right, SpriteAnimation, Twoway, Collision, Gravity, Tween, Keyboard')
    ...
    .reel('moveRight', 500, [[0,0], [1, 0], [2, 0], [3, 0], [4, 0], [0, 1], [1, 1]])
    .reel('moveLeft', 500, [[1, 2], [2, 2], [3, 2], [4, 2], [0, 3], [1, 3], [2, 3]])
    .reel('jumpLeft', 1500,[[4, 3], [0, 4], [1, 4]])
    .reel('jumpRight', 1500, [[3, 1], [4, 1], [0, 2]])
    .reel('standing', 100, [[2, 1], [3, 1]])
    .twoway(200, 510)
    .gravity('FloorTile')
    ...
    .bind('LandedOnGround', function (ground) {
      if (this.isJumping) {
        ...
        //this.sprite(0, this.getReel().id === 'moveRight' ? 3 : 1);
...
function displayGameOver(){
  ...
  Crafty.e("GameOver, 2D, DOM, Text")
    .attr({ x: 280 - Crafty.viewport._x, y: 280, z:100, w: 600 })
    .text("YA GOT BURNED BOI").textColor('#000000').textFont({ size: '36px', weight: 'bold', family: 'Courier New' });
}
...
Crafty.bind('EnterFrame', function(){
  if(!__STATE.gameStarted || __STATE.levelComplete)
    ...
    if( _player.y >= SCREENHEIGHT ){
      console.log('Player fall down, get fucked!');
