...
var playerSprite = { 'sprites': { 'img/spritey guy.png': { tile: 68, tileh: 80, map: { man_left: [0, 1], man_right: [0, 2], jump_right: [6, 4] } } } };
...
function loadBackground () {
  ...
  Crafty.e('BG, 2D, DOM, Image')
    ...
    .image( 'img/DesertBG.png', 'repeat-x' );
}
...
function spawnPlayer (){
  _player = Crafty.e('Player, 2D, DOM, man_right, SpriteAnimation, Twoway, Collision, Gravity, Tween, Keyboard')
    ...
    .reel('moveRight', 500, [[0,0] , [1,0] , [2,0] , [3,0] , [4,0] , [4,0] , [3,0] , [2,0] , [1,0]])
    .reel('moveLeft', 500, [[2,2] , [1,2] , [3,1] , [4,1] , [3,1] , [3,1] , [4,1] , [3,1] , [1,2] , [2,2]])
    .twoway(200, 510)
    .gravity('FloorTile')
    .reel('jumpRight' , 500, [[1,3] , [2,3] , [3,3]])
    .reel('jumpLeft', 500, [[3,4] , [2,4] , [1,4]])
...
if(__STATE.level >= _tileMap.lenght){
  displayGameComplete();
  return;
}
...
function displayGameOver(){
  ...
  Crafty.e("GameOver, 2D, DOM, Text")
    ...
    .text("wait you died?! how'd that happen").textColor('#FF0000').textFont({ size: '36px', weight: 'bold', family: 'Courier New' });
}
...
function displayLevelComplete(){
  ...
  Crafty.e("LevelComplete, 2D, DOM, Text")
    ...
    .text("LEVEL "+__STATE.level+" You Win").textColor('#3333FF').textFont({ size: '36px', weight: 'bold', family: 'Courier New' });
}
...
function displayGameComplete(){
  ...
  Crafty.e("GameComplete, 2D, DOM, Text")
    .attr({ x: 270 - Crafty.viewport._x, y: 280, z:100, w: 350 })
    .text("Wait you didn't die congrats").textColor('#33FF33').textFont({ size: '36px', weight: 'bold', family: 'Courier New' });
}
...
if( _player.y >=SCREENHEIGHT ){
  ...
  console.log("PLayer fell down, go boom")
}
