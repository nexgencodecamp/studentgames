...
var _tileMap = [
  ...
  },
  {
    ...
    "map": [
      ...
      [[0,4],[0,4],[0,19],[0,1],[0,3],[0,1],[0,6],[0,1],[0,4]],
  ...
  },
  {
    ...
    "map" : [
      ...
      [[0,48]],
      [[0,8], [0,6],[4,1], [0,6]],
      [[0,20], [4,1], [0,8], [8,1], [0,8]],
      [[0,48]],
      [[0,1], [0,4], [0,13], [8,1], [0,22]],
      [[0,48]],
      [[1,30], [0,2], [1,16]]
    ]
  },
  {
    ...
    "map" : [
      [[0, 12], [8, 1], [0, 28],],
      [[0, 48]],
      [[0, 6], [8, 1], [0, 8],[0,1], [4, 1], [0,21],],
      [[0, 48]],
      [[0, 20], [0,3], [8,1],[0,17],],
      [[0, 4],[4, 1],[0,16], [0,8], [0,16],],
      [[8, 1],[0, 9],[4, 1],[0,27]],
      [[0,48]]
    ]
  },
...
var assets = {'tiles': ['img/Ground.png', 'img/platform 1.png', 'img/platform 2.png']};
var playerSprite = { 'sprites': { 'img/Main Player.png': { tile: 67, tileh: 78, map: { man_left: [1, 1], man_right: [1, 1], jump_right: [1, 1] } } } };
...
function initialiseGame () {
  Crafty.load(assets, function(){
    ...
    Crafty.audio.add("soundtrack", "sounds/Birdland.mp3");
...
function spawnPlayer (){
  _player = Crafty.e('Player, 2D, DOM, man_right, SpriteAnimation, Twoway, Collision, Gravity, Tween, Keyboard')
    ...
    .reel('moveRight', 1000, [[0, 0], [1, 0], [2, 0], [3, 0], [4, 0], [0, 1]])
    .reel('moveLeft', 1000, [[0, 2], [1, 2], [2, 2], [3, 2], [4, 2], [0, 3]])
    .reel('jumpRight', 1000, [[3, 1]])
    .reel('jumpLeft', 1000, [[3, 3]])
    .reel('stillLeft', 1000, [[2,3]])
    .reel('stillRight', 1000, [[1,1]])
    .twoway(200, 510)
    .gravity('FloorTile')
    ...
    .bind('KeyDown',
      function (e) {
        ...
        if (e.key === Crafty.keys.D && !this.isJumping) {
          this.animate('moveRight', -1);
        }
        else if (e.key === Crafty.keys.A && !this.isJumping) {
    ...
    .bind('KeyUp',
      function (e) {
        ...
        if ((this.isPlaying('moveRight') && e.key === Crafty.keys.D) ||
          (this.isPlaying('moveLeft') && e.key === Crafty.keys.A)) {
          this.pauseAnimation();
          this.resetAnimation();
          /* Check/Set the player's moving state */
          if (e.key === Crafty.keys.D || e.key === Crafty.keys.A) {
            this.isMoving = false;
            return;
          }
        }
        /* Kickstart animation if we need to */
        if (this.isDown('D') && !this.isJumping) {
          this.animate('moveRight', -1);
          this.isMoving = true;
        }
        else if (this.isDown('A') && !this.isJumping) {
          this.animate('moveLeft', -1);
          this.isMoving = true;
        }
    ...
    .bind('LandedOnGround', function (ground) {
      if (this.isJumping) {
        ...
        //this.sprite(0, this.getReel().id === 'moveRight' ? 1 : 1);
      ...
      if (tileType === 1) {
        for(var i=0; i < tileNum; i++){
          Crafty.e('FloorTile, 2D, DOM, Image, Collision')
            ...
            .image(Crafty.assets['img/Ground.png'].src);
          xPos += 80;
        }
      }
      else{
...
function displayTimeLeft () {
  ...
  _timeLeftDisplay = Crafty.e("2D, DOM, Text")
    ...
    .text("Time: " + __STATE.timeLeft + "s")
    .textColor('#FF0000')
    .textFont({ size: '20px', weight: 'bold', family: 'Comic Sans MS' });
}
...
function displayScore () {
  ...
  _scoreDisplay = Crafty.e("2D, DOM, Text")
    .attr({ x: 10 - Crafty.viewport._x, y: 20 })
    .text(pad("Score: " + __STATE.currentScore, 3))
    .textColor('#FF0000')
    .textFont({ size: '20px', weight: 'bold', family: 'Comic Sans MS' });
}
...
function displayGameOver(){
  ...
  Crafty.e("GameOver, 2D, DOM, Text")
    .attr({ x: 130 - Crafty.viewport._x, y: 280, z:300, w: 600 })
    .text("You Got Wrecked Mate").textColor('#FF0000').textFont({ size: '50px', weight: 'bold', family: 'Comic Sans MS' });
}
...
function displayLevelComplete(){
  ...
  Crafty.e("LevelComplete, 2D, DOM, Text")
    .attr({ x: 160 - Crafty.viewport._x, y: 280, z:300, w: 500 })
    .text("LEVEL "+__STATE.level+" COMPLETE").textColor('#0000FF').textFont({ size: '50px', weight: 'bold', family: 'Comic Sans MS' });
}
...
function displayGameComplete(){
  ...
  Crafty.e("GameComplete, 2D, DOM, Text")
    .attr({ x: 260 - Crafty.viewport._x, y: 280, z:300, w: 500 })
    .text("YOU WIN!!!").textColor('#0000FF').textFont({ size: '50px', weight: 'bold', family: 'Comic Sans MS' });

  Crafty.e("GameComplete, 2D, DOM, Text")
    .attr({ x: 160 - Crafty.viewport._x, y: 380, z:300, w: 700 })
    .text("A Game By Josh Hort").textColor('#0000FF').textFont({ size: '50px', weight: 'bold', family: 'Comic Sans MS' });
}
...
if ( ! __STATE.gameOver && _player.y >= SCREENHEIGHT ){
  ...
  //console.log("Your Player Fell Down and Hit His Head")
}
